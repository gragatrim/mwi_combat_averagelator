// =============================================================================
// MWI Combat Averagelator - Trigger
// =============================================================================
// Ported from mwi_combat_sim trigger.js. All data lookups use injected GameData
// instead of importing JSON directly.

import type {
  GameData,
  TriggerData,
  TriggerDependencyHrid,
  TriggerConditionHrid,
  TriggerComparatorHrid,
  CombatDetails,
  BuffInstance,
} from "./types";

// ---------------------------------------------------------------------------
// Minimal interface for a combat unit as seen by Trigger methods.
// This avoids a circular dependency on a full CombatUnit class.
// ---------------------------------------------------------------------------

export interface TriggerCombatUnit {
  combatDetails: CombatDetails;
  combatBuffs: Record<string, BuffInstance | number | boolean | undefined>;
  isStunned: boolean;
  stunExpireTime: number | null;
  isBlinded: boolean;
  blindExpireTime: number | null;
  isSilenced: boolean;
  silenceExpireTime: number | null;
}
...
class Trigger {
  public dependencyHrid: TriggerDependencyHrid | string;
  public conditionHrid: TriggerConditionHrid | string;
  public comparatorHrid: TriggerComparatorHrid | string;
  public value: number;
  ...
  static createFromDTO(gameData: GameData, dto: TriggerData): Trigger {
  ...
  isActive(
    ...
    const depDetail =
      this.gameData.combatTriggerDependencyDetailMap[this.dependencyHrid];
    if (depDetail && depDetail.isSingleTarget) {
  ...
  private isActiveSingleTarget(
    source: TriggerCombatUnit,
    target: TriggerCombatUnit | null,
    currentTime: number,
  ): boolean {
    let dependencyValue: number | boolean;
  ...
  private isActiveMultiTarget(
    friendlies: TriggerCombatUnit[],
    enemies: TriggerCombatUnit[] | null,
    currentTime: number,
  ): boolean {
    let dependency: TriggerCombatUnit[];
    switch (this.dependencyHrid) {
  ...
  private getDependencyValue(
    ...
    switch (this.conditionHrid) {
      // --- Exact-match buff conditions (uniqueHrid == /buff_uniques/<name>) ---
      ...
      case "/combat_trigger_conditions/weaken": {
        ...
        return source.combatBuffs[buffHrid] as number | boolean;
      }

      // --- Prefix-match buff conditions (uniqueHrid starts with /buff_uniques/<name>) ---
      ...
      case "/combat_trigger_conditions/enrage": {
        ...
        buffPrefix += this.conditionHrid.slice(
          this.conditionHrid.lastIndexOf("/"),
        );
        const buffs = Object.keys(source.combatBuffs).filter((buff) =>
          buff.startsWith(buffPrefix),
        );
        return source.combatBuffs[buffs?.[0]] as number | boolean;
      }
  ...
  private compareValue(dependencyValue: number | boolean): boolean {
    switch (this.comparatorHrid) {
      case "/combat_trigger_comparators/greater_than_equal":
        return (dependencyValue as number) >= this.value;
